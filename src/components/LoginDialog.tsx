...
export default function LoginDialog({ trigger, open: controlledOpen, onOpenChange }: LoginDialogProps) {
  ...
  async function handleReownLogin() {
    if (setOpen) {
      setOpen(false)
    }
    await openAppKit()
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex flex-col gap-3 py-4">
            <Button variant="outline" className="flex h-12 w-full items-center justify-start gap-3 px-4" onClick={handleEmailLogin}>
              <MailIcon className="size-5" />
              <span>Log in with email</span>
            </Button>

            <Button variant="outline" className="flex h-12 w-full items-center justify-start gap-3 px-4" onClick={handlePhoneLogin}>
              <PhoneIcon className="size-5" />
              <span>Log in with phone</span>
            </Button>

            <Button variant="outline" className="flex h-12 w-full items-center justify-start gap-3 px-4" onClick={handleReownLogin}>
              {/* Google Icon */}
